...
class DryRequestsCollectors_test extends TestCase {
	...
	public function testGETDryRequest(){
		...
		$ActualDryRequest = TheKof\SurveyMonkey::surveys($fake_survey_id)->collectors()->get_dry();
		...
		$ActualDryRequest = TheKof\SurveyMonkey::surveys($fake_survey_id)->collectors()->get_dry(1);
		...
		$ActualDryRequest = TheKof\SurveyMonkey::surveys($fake_survey_id)->collectors()->get_dry(2,10);
	...
	public function testPOSTDryRequest(){
		...
		$ExpectedDryRequest = new \TheKof\Util_DryRequest(Env::$survey_monkey_config['access_token']);
		$ExpectedDryRequest->url($expected_url);
		$ExpectedDryRequest->method(\TheKof\ThirdPartyWrappers_HTTPClient_a::METHOD_POST);
		$ExpectedDryRequest->body($raw_data);

		$CollectorModel = new \TheKof\Model_Collector($raw_data);
		$ActualDryRequest = TheKof\SurveyMonkey::surveys($fake_survey_id)->collectors()->post_dry($CollectorModel);
		
		$this->assertInstanceOf(\TheKof\Util_DryRequest::class, $ActualDryRequest,'post_dry method must return a \TheKof\DryRequest object');
		$this->assertEquals($ExpectedDryRequest,    $ActualDryRequest,            'response structure is not same');
		$this->assertEquals($expected_url,          $ActualDryRequest->url(),     'url does not match');
